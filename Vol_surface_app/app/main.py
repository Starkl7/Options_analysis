...
def fetch_option_chain(ticker):
    ...
    try:
        ...
        # For now, only picking the nearest expiration
        ...
        calls['report_time'] = sysdate
        puts['report_time'] = sysdate

        calls['type'] = 'call'
        puts['type'] = 'put'

        options = pd.concat([calls, puts], ignore_index=True)
        options['expiry_date'] = expiry

        option_chain = pd.concat([option_chain, options], ignore_index=True)

    except Exception as e:
        print(f"Error fetching option chain for {ticker}: {e}", exc_info=True)

def load_options_live_data(ticker = 'AAPL'):
    options_data = fetch_option_chain(ticker)
...
def parse_options_data(ticker='AAPL', report_date='2026-01-26 11:00:00'):
    if report_date == "LIVE":
        options_data = load_options_live_data(ticker)
    else:
